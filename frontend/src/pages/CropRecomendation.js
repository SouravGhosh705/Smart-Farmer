...
const CropRecomendation = () => {
	...
	const formSubbmit = async () => {
		...
		setLoading(true);
		await API.post("/crop_prediction", data)
			.then((res) => {
				...
				const transformedData = {
					...
					static_info: {
						...
						soilBarChartLabel: ["N", "P", "K", "pH"],
						soilBarChartUserValue: [parseFloat(Nitrogen), parseFloat(Phosphoras), parseFloat(Potassium), parseFloat(Ph)],
						weatherBarChartLabel: ["Temperature", "Humidity"],
						weatherBarChartUserValue: [25, 65] // Default values
					}
				};
			...
			.catch((err) => {
				...
				alert('Error getting crop recommendations. Please try again.');
			});
		setshowGrid(true);
	};
	const toggalSwitch = () => {
		...
		<>
			...
			{showGrid ? (
				<>
					...
					<Container fluid className='contant-container'>
						<Row>
							{cropRecommendationData.Top5CropInfo && cropRecommendationData.Top5CropInfo.map((crop, idx) => {
								return (
									<>
										<Col key={idx}>
											<Card
			...
			{showGrid && cropRecommendationData.static_info ? (
				<Container fluid className='contant-container'>
					<Row>
